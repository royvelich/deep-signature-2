...
def contrastive_tuplet_loss(a,p,n):
    # loss = torch.log(1 + torch.exp(torch.linalg.matrix_norm(a-p) - torch.linalg.matrix_norm(a-n)))/a.size(0)
    norm_ap = torch.linalg.norm(a - p, dim=0)
    norm_an = torch.linalg.norm(a - n, dim=0)

    loss = torch.log(1 + torch.exp(norm_ap - norm_an))
    loss = torch.sum(loss) / a.size(0)
...
def loss_contrastive_plus_pearson_correlation_k1_k2(a,p,n, device='cpu'):
    return (2*contrastive_tuplet_loss(a,p,n) +calculate_pearson_k1_k2_loss_vectorized(torch.cat([a,p,n], dim=1).T, device))
...
def loss_contrastive_plus_pearson_correlation_k1__greater_k2_hinge_loss(a,p,n, device='cpu'):
    return (5*contrastive_tuplet_loss(a,p,n) +calculate_pearson_k1_k2_loss_vectorized(torch.cat([a,p,n], dim=1).T, device) + 0.1*k1_greater_k2_hinge_loss(torch.cat([a,p,n], dim=1).T))
