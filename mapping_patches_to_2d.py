...
def map_patches_to_2d():
    ...
    model_path = "C:/Users\galyo\Downloads\model_point_transformer_3_layers_width_128_train_non_uniform_samples_with_pearson_loss-epoch=04.ckpt"
    ...
    meshes_names = ["peak30", "saddle30", "peak230", "2saddle30", "peak3_non_uniform"]
    output_points = []
    output_points2 = []
    output_points3 = []
    output_points4 = []
    ...
    plt.scatter(output_points2[:, 0], output_points2[:, 1], c='red', label='Saddle Points2: 2*x ** 2 - y ** 2')
    plt.scatter(output_points3[:, 0], output_points3[:, 1], c='green', label='Peak2 Points: 7*x ** 2 + y ** 2')
    plt.scatter(output_points4[:, 0], output_points4[:, 1], c='yellow', label='Saddle Points: 0.5*x ** 2 - 2*y_grid ** 2')
